...
A top level `_hints: {my_type: {...}, ...}` field may be given, which for a subset of the types gives key/value hints on how to represent the datatype in the programming language the consumes the file. The values are of type `any()`, and the interpretation is left up to each implementation.
